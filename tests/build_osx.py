...
from helpers._tns_lib import TNS_PATH, create_project, prepare, \
...
class BuildiOS(unittest.TestCase):
    ...
    def test_200_build_ios_release(self):
        create_project_add_platform(proj_name="TNS_App", \
            ...
            framework_path=IOS_RUNTIME_SYMLINK_PATH, \
            symlink=True)
        output = run_aut(TNS_PATH + " build ios --path TNS_App --release")
        assert "Project successfully prepared" in output
        assert "CONFIGURATION Release" in output
        assert "build/emulator/TNSApp.app" in output
        assert "** BUILD SUCCEEDED **" in output
    ...
    def test_201_build_ios_fordevice(self):
        create_project_add_platform(proj_name="TNS_App", \
            ...
            framework_path=IOS_RUNTIME_SYMLINK_PATH, \
            symlink=True)
        output = run_aut(TNS_PATH + " build ios --path TNS_App --forDevice")
        assert "Project successfully prepared" in output
    ...
    def test_213_build_ios_platform_not_added(self):
        ...
        output = run_aut(TNS_PATH + " build ios --path TNS_AppNoPlatform")
        assert "Project successfully prepared" in output
    ...
    def test_300_build_ios_with_dash(self):
        ...
        output = run_aut(TNS_PATH + " build ios --path tns-app")
        assert "Project successfully prepared" in output
        assert "build/emulator/tnsapp.app" in output
        assert "** BUILD SUCCEEDED **" in output
        assert "Project successfully built" in output
    ...
    def test_302_build_ios_with_ios_in_path(self):
        ...
        # Verify project builds
        output = run_aut(TNS_PATH + " build ios --path my-ios-app")
        assert "Project successfully prepared" in output
    ...
    def test_310_build_ios_with_copy_to(self):
        create_project_add_platform(
            ...
            framework_path=IOS_RUNTIME_SYMLINK_PATH, \
            symlink=True)
        output = run_aut(TNS_PATH + " build ios --path TNS_App --copy-to ./")
        assert "Project successfully prepared" in output
        assert "build/emulator/TNSApp.app" in output
        assert "** BUILD SUCCEEDED **" in output
        assert "Project successfully built" in output
