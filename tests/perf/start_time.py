...
class PerfTests(BaseClass):
    ...
    @staticmethod
    def run_perfs(self, perf_loop, app_id, apk, type_of_run):
        start_time = 0
        second_start = 0
    ...
    def test_start_time(self, demo, config, device_name, device_id, first_start, second_start):
        ...
        if old_way_of_testing_performance is False:
            expected_time = PerfTests.run_perfs(self, perf_loop, app_id, release_apk, "expected time")
        ...
        actual_time = PerfTests.run_perfs(self, perf_loop, app_id, apk, "actual time")
